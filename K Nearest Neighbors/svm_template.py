...
def load_data(csv_file_path, encode_cat = False, outlier_removal = True, feature_engineering = True):
...
def cross_validation(x_train, y_train, params, num_folds=3):
    ...
    return df, df['cv_test_accuracy'].max(), models[df['cv_test_accuracy'].idxmax()]
...
def train_and_select_model(training_csv):
    ...
    x,y, xcols = load_data(training_csv, encode_cat = True, outlier_removal = True, feature_engineering = False)
    ...
    param_set = [
                    ...
                    #{'kernel': 'poly', 'C': 100, 'degree': 1},

        ]



    table, best_score, best_model = cross_validation(x, y, param_set)
...
def predict(test_csv, trained_model):
    x_test, _ , xcols = load_data(testing_csv, encode_cat = True, outlier_removal = True, feature_engineering = False)
    x_test = pd.DataFrame(MinMaxScaler().fit_transform(x) , columns = xcols)
